...
describe('HTML browser Agent bridge', () => {
  it('requires the exact save approval and consumes it after one save', async () => {
    ...
    const bridge = createHtmlBrowserAgentBridge({ client, documentId: 'html-1' as never, revision: 1 as never })
    const save = vi.fn().mockResolvedValue({ ok: true, summary: 'Saved.', warnings: [] })
    bridge.attachEditor({
      editorType: 'html', capabilities: () => ({ editorType: 'html', commands: [], canUndo: false, canSave: true, canExport: false }),
      snapshot: vi.fn(), read: vi.fn(), propose: vi.fn(), apply: vi.fn(), verify: vi.fn(), undo: vi.fn(), save, export: vi.fn(),
    } as never)
    const target = { sessionId: 'session-1', documentId: 'html-1', editorType: 'html', revision: 1, clientId: 'client-1' }
    const sendSave = (operationId: string, approval?: { id: string; planHash: string }) =>
      (client.frames.push as never)({ type: 'editor:request', protocolVersion: 1, id: `request-${operationId}`, target: { ...target, operationId }, command: 'save_html', arguments: { inPlace: true }, approval })
    ...
    expect(client.sent).toContainEqual(expect.objectContaining({
      type: 'editor:result', target: expect.objectContaining({ operationId: 'missing' }),
      result: expect.objectContaining({ ok: false, warnings: [expect.objectContaining({ code: 'APPROVAL_INVALID' })] }),
    }))
    expect(client.sent).toContainEqual(expect.objectContaining({
      type: 'editor:result', target: expect.objectContaining({ operationId: 'wrong' }),
      result: expect.objectContaining({ ok: false, warnings: [expect.objectContaining({ code: 'APPROVAL_INVALID' })] }),
    }))
  })
})
