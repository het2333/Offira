...
it.each([70, 255])(
  ...
  async (kib) => {
    ...
    const operations = [{ op: 'replaceText', find: 'Original', replace: 'x'.repeat(kib * 1024) }]
    s.bridge.attachEditor({
...
it.each(['getItem', 'setItem', 'removeItem'])(
  ...
  async (method) => {
    ...
    expect(() => s.emit(s.frame('read_markdown'))).not.toThrow()
    await vi.waitFor(() => expect(s.results()).toHaveLength(1))
    expect(s.results()[0].result.ok).toBe(true)
    expect(reads).toBe(1)
    s.bridge.dispose()
...
it('evicts completed promises and journals beyond the per-document limit', async () => {
  ...
  const first = s.frame('read_markdown')
