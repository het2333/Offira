...
class EffectiveToolCatalog {
  ...
  schemas(scope?: object): Array<{ name: string }> {
    if (scope !== agentScope) return [...OFFICE_TOOL_NAMES, 'bash'].map((name) => ({ name }))
...
describe('Office-only Agent capability policy', () => {
  ...
  it('removes inherited shell, filesystem, search, skill, and web tools from the effective catalog', () => {
    ...
    configureOfficeToolScope({ tools }, 'sheets', agentScope)
  ...
  it('exposes only Docs tools for a Docs session', () => {
    ...
    configureOfficeToolScope({ tools }, 'docs', agentScope)
  ...
  it('exposes only Markdown tools for a Markdown session', () => {
    ...
    configureOfficeToolScope({ tools }, 'markdown', agentScope)

    expect(tools.schemas(agentScope).map(({ name }) => name).sort()).toEqual([
      ...MARKDOWN_TOOL_NAMES,
    ].sort())
  ...
  it('exposes only HTML tools for an HTML session', () => {
    const tools = new EffectiveToolCatalog()
    configureOfficeToolScope({ tools }, 'html', agentScope)
    expect(tools.schemas(agentScope).map(({ name }) => name).sort()).toEqual([
      ...HTML_TOOL_NAMES,
    ].sort())
  ...
  it('exposes only PDF tools for a PDF session', () => {
    const tools = new EffectiveToolCatalog()
    configureOfficeToolScope({ tools }, 'pdf', agentScope)
    expect(
      tools
        .schemas(agentScope)
        .map(({ name }) => name)
        .sort(),
    ).toEqual([...PDF_TOOL_NAMES].sort())
  ...
  it('rejects an unknown editor kind instead of widening the catalog', () => {
    const tools = new EffectiveToolCatalog()
    expect(() => configureOfficeToolScope({ tools }, 'unknown', agentScope)).toThrow(
      /unsupported Office editor/i,
    )
  })
})
