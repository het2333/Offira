...
function bounded(value: AgentToolResult): AgentToolResult {
...
function proposal(result: AgentToolResult): {
  operationId: string
  proposal: AgentApprovalProposal
} {
  ...
  return {
    ...
    proposal: {
      ...
      targets: Array.isArray(data.targets)
        ? data.targets.filter((v): v is string => typeof v === 'string')
        : [],
...
export function createHtmlTools(bridge: HtmlToolBridge): ToolDefinition[] {
  const read = defineTool({
    ...
    async execute(_args, execution) {
      return bounded(await bridge.request('read_html', {}, execution)) as unknown as JsonValue
    },
  })
  const apply = defineTool({
    name: 'apply_html_operations',
    description:
      'Apply one ordered batch of existing GenOffice HTML apply_ops operations after exact user approval.',
    parameters: {
      operations: {
        type: 'array',
        required: true,
    ...
    async execute(args, execution) {
      ...
      if (!approval.approved || approval.approvalId === undefined)
        return approvalDenied('HTML mutation') as unknown as JsonValue
      return bounded(
  ...
  const save = defineTool({
    ...
    async execute(_args, execution) {
      ...
      const approval = await bridge.approve('save_html', proposal, execution)
      if (!approval.approved || approval.approvalId === undefined) {
        return approvalDenied('HTML save') as unknown as JsonValue
      }
      return parseAgentToolResult(
