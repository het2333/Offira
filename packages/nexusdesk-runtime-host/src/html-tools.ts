...
function proposal(result: AgentToolResult): { operationId: string; proposal: AgentApprovalProposal } {
  ...
  return { operationId: data.operationId, proposal: { planHash: data.planHash, summary: typeof data.summary === 'string' ? data.summary : result.summary, targets: Array.isArray(data.targets) ? data.targets.filter((v): v is string => typeof v === 'string') : [], warnings: result.warnings } }
}
